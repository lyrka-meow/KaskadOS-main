...
Regalia is the integrated VPN control service of MacqueenDE, designed for
terminal use, automation, and direct desktop-shell integration. Its
authoritative source lives in `components/regalia/` of KaskadOS-main.
...
Regalia is built together with MacqueenDE by
`scripts/prepare-live-profile.sh`. Its binaries, systemd units, polkit rule and
a checksum-verified official `sing-box` are included in the ISO and copied by
Calamares without downloading the VPN component during installation.

The per-user `regaliad` service starts automatically at desktop login.
MacqueenDE talks to it through the private socket in `$XDG_RUNTIME_DIR` and
provides connection, subscription, server and application-routing controls in
its own settings. The scripts in `installer/` and `packaging/` remain available
as development tooling but are not used by the KaskadOS installer.
