...
repo=${REGALIA_GITHUB_REPO:-lyrka-meow/KaskadOS-main}
component_path=${REGALIA_REPOSITORY_PATH:-components/regalia}
release_tag=${REGALIA_RELEASE_TAG:-regalia-rolling}
...
tmp_dir=$(mktemp -d)
checkout_dir="$tmp_dir/kaskados"
source_dir="$checkout_dir/$component_path"
...
{
    local url
    url="https://raw.githubusercontent.com/$repo/main/$component_path/installer/install-release-payload.sh"
...
install_source()
{
    ...
    git clone --depth 1 --branch main "https://github.com/$repo.git" "$checkout_dir"
    [[ -d "$source_dir" ]] || {
        echo "Regalia source directory is missing: $component_path" >&2
        return 1
    }
