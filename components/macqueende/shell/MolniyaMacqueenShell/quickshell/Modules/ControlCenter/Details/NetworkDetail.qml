...
Rectangle {
    ...
    implicitHeight: {
        ...
        if (NetworkService.wifiToggling)
            return headerRow.height + hotspotContentHeight + vpnContentHeight + qualityContentHeight + wifiToggleContent.height + Theme.spacingM;
        if (NetworkService.wifiEnabled)
            return headerRow.height + hotspotContentHeight + vpnContentHeight + qualityContentHeight + wifiContent.height + Theme.spacingM;
        return headerRow.height + hotspotContentHeight + vpnContentHeight + qualityContentHeight + wifiOffContent.height + Theme.spacingM;
    }
    ...
    readonly property int hotspotContentHeight: currentPreferenceIndex === 1 && NetworkService.hotspotAvailable && !connectionQuality.expanded ? 56 + Theme.spacingS : 0
    readonly property int vpnContentHeight: regaliaVpnRow.height + Theme.spacingS
    ...
    function openVpnSettings() {
        PopoutService.closeControlCenter();
        PopoutService.openSettingsWithTab("network_vpn_overview");
    }
    ...
    Item {
        ...
        visible: currentPreferenceIndex === 1 && NetworkService.hotspotAvailable && !connectionQuality.expanded
        height: visible ? 56 : 0
    ...
    Rectangle {
        ...
        height: 56
        radius: Theme.cornerRadius
        color: vpnRowMouse.containsMouse ? Theme.primaryHoverLight : Theme.surfaceLight
        border.width: RegaliaService.enabled ? 2 : 1
        border.color: RegaliaService.enabled ? Theme.primary : Theme.outlineLight

        Row {
            anchors.left: parent.left
            anchors.right: vpnControls.left
            anchors.leftMargin: Theme.spacingM
            anchors.rightMargin: Theme.spacingS
            anchors.verticalCenter: parent.verticalCenter
            spacing: Theme.spacingS
            ...
            Rectangle {
                ...
                radius: 17
                anchors.verticalCenter: parent.verticalCenter
                color: RegaliaService.enabled ? Theme.primaryHoverLight : Theme.surfaceVariant

                DankIcon {
                    anchors.centerIn: parent
                    name: RegaliaService.enabled ? "vpn_lock" : "vpn_key_off"
                    size: 19
                    color: RegaliaService.enabled ? Theme.primary : Theme.surfaceVariantText
                }
            }
            ...
            Column {
                ...
                StyledText {
                    ...
                    font.pixelSize: Theme.fontSizeMedium
                    font.weight: Font.Medium
                    color: RegaliaService.enabled ? Theme.primary : Theme.surfaceText
                    elide: Text.ElideRight
                }
                ...
                StyledText {
                    width: parent.width
                    text: {
                        if (RegaliaService.availabilityState === "checking")
                            return I18n.tr("Checking Regalia");
                        if (!RegaliaService.installed)
                            return I18n.tr("Regalia is not installed");
                        if (!RegaliaService.available)
                            return I18n.tr("Regalia service is not running");
                        if (!RegaliaService.configurationReady)
                            return I18n.tr("Choose a VPN server in Settings");
                        if (RegaliaService.busy)
                            return RegaliaService.enabled ? I18n.tr("Disconnecting...") : I18n.tr("Connecting...");
                        if (RegaliaService.enabled)
                            return RegaliaService.activeServer?.name || I18n.tr("Connected");
        ...
        Row {
            ...
            DankActionButton {
                ...
                iconName: "settings"
                iconColor: Theme.surfaceVariantText
                onClicked: root.openVpnSettings()
            }

            DankToggle {
                anchors.verticalCenter: parent.verticalCenter
                checked: RegaliaService.enabled
                enabled: RegaliaService.available && RegaliaService.configurationReady && !RegaliaService.busy
        ...
        MouseArea {
            id: vpnRowMouse
            anchors.fill: parent
            anchors.rightMargin: vpnControls.width + Theme.spacingM * 2
            hoverEnabled: true
            cursorShape: Qt.PointingHandCursor
            onClicked: root.openVpnSettings()
        }
    ...
    ConnectionQualityCard {
        ...
        anchors.top: regaliaVpnRow.bottom
        anchors.left: parent.left
        anchors.right: parent.right
        anchors.leftMargin: Theme.spacingM
        anchors.rightMargin: Theme.spacingM
