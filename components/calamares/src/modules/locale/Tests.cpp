...
private Q_SLOTS:
    ...
    void testOfflineTimezoneDefaults();
    void testOfflineTimezonePriority();
...
LocaleTests::initTestCase()
{
    ...
    {
        (void)new Calamares::JobQueue( this );
    }
}
...
static QVariantMap
offlineLocaleConfiguration()
{
    const QFileInfo fi( QStringLiteral( "%1/locale.conf" ).arg( BUILD_AS_TEST ) );
    bool ok = false;
    QVariantMap configuration = Calamares::YAML::load( fi, &ok );
    if ( !ok )
    {
        return {};
    }
    configuration.insert( QStringLiteral( "adjustLiveTimezone" ), false );
    return configuration;
}

void
LocaleTests::testOfflineTimezoneDefaults()
{
    const QVariantMap configuration = offlineLocaleConfiguration();
    QVERIFY( !configuration.isEmpty() );
    QVERIFY( !configuration.contains( QStringLiteral( "geoip" ) ) );

    Config c;
    c.setConfigurationMap( configuration );

    QCOMPARE( c.defaultTimezoneForLocale( QStringLiteral( "ru" ) ).asString(), QStringLiteral( "Europe/Moscow" ) );
    QCOMPARE( c.defaultTimezoneForLocale( QStringLiteral( "ru_RU.UTF-8" ) ).asString(),
              QStringLiteral( "Europe/Moscow" ) );
    QCOMPARE( c.defaultTimezoneForLocale( QStringLiteral( "en_GB.UTF-8" ) ).asString(),
              QStringLiteral( "Europe/London" ) );
    QCOMPARE( c.defaultTimezoneForLocale( QStringLiteral( "es-AR" ) ).asString(),
              QStringLiteral( "America/Argentina/Buenos_Aires" ) );
    QCOMPARE( c.defaultTimezoneForLocale( QStringLiteral( "zh-HK" ) ).asString(),
              QStringLiteral( "Asia/Hong_Kong" ) );
    QCOMPARE( c.defaultTimezoneForLocale( QStringLiteral( "sr_RS.UTF-8@latin" ) ).asString(),
              QStringLiteral( "Europe/Belgrade" ) );
    QCOMPARE( c.defaultTimezoneForLocale( QStringLiteral( "unknown" ) ).asString(),
              QStringLiteral( "Europe/Moscow" ) );

    for ( const QString& language : Calamares::Locale::availableLanguages() )
    {
        QVERIFY2( c.hasTimezoneForLocale( language ),
                  qPrintable( QStringLiteral( "No explicit offline timezone rule for %1" ).arg( language ) ) );
        const auto timezone = c.defaultTimezoneForLocale( language );
        QVERIFY2( timezone.isValid(), qPrintable( QStringLiteral( "No offline timezone for %1" ).arg( language ) ) );
        QVERIFY2( c.zonesModel()->find( timezone.region(), timezone.zone() ),
                  qPrintable( QStringLiteral( "Invalid offline timezone %1 for %2" )
                                  .arg( timezone.asString(), language ) ) );
    }
}

void
LocaleTests::testOfflineTimezonePriority()
{
    const QVariantMap configuration = offlineLocaleConfiguration();
    QVERIFY( !configuration.isEmpty() );

    auto* storage = Calamares::JobQueue::instance()->globalStorage();
    QVERIFY( storage );
    Calamares::Locale::insertGS( *storage, QStringLiteral( "LANG" ), QStringLiteral( "ru" ) );

    Config automatic;
    automatic.setConfigurationMap( configuration );
    automatic.setCurrentLocation();
    QCOMPARE( automatic.currentTimezoneCode(), QStringLiteral( "Europe/Moscow" ) );

    Calamares::Locale::insertGS( *storage, QStringLiteral( "LANG" ), QStringLiteral( "en_GB" ) );
    automatic.setCurrentLocation();
    QCOMPARE( automatic.currentTimezoneCode(), QStringLiteral( "Europe/London" ) );

    automatic.setCurrentLocation( QStringLiteral( "Asia" ), QStringLiteral( "Tokyo" ) );
    Calamares::Locale::insertGS( *storage, QStringLiteral( "LANG" ), QStringLiteral( "ru" ) );
    automatic.setCurrentLocation();
    QCOMPARE( automatic.currentTimezoneCode(), QStringLiteral( "Asia/Tokyo" ) );
}
