...
static QStringList
localeLookupKeys( QString localeName )
{
    localeName = localeName.trimmed();

    const int modifierPosition = localeName.indexOf( '@' );
    const int encodingPosition = localeName.indexOf( '.' );
    if ( encodingPosition >= 0 && ( modifierPosition < 0 || encodingPosition < modifierPosition ) )
    {
        const int encodingEnd = modifierPosition >= 0 ? modifierPosition : localeName.size();
        localeName.remove( encodingPosition, encodingEnd - encodingPosition );
    }

    QStringList keys;
    auto appendKey = [ &keys ]( QString key )
    {
    ...
    appendKey( localeName );

    QString normalized = localeName;
    normalized.replace( '-', '_' );
    appendKey( normalized );

    const int normalizedModifierPosition = normalized.indexOf( '@' );
    QString withoutModifier = normalized;
    QString modifier;
    if ( normalizedModifierPosition >= 0 )
    {
        modifier = normalized.mid( normalizedModifierPosition );
        withoutModifier.truncate( normalizedModifierPosition );
        const int territoryPosition = withoutModifier.indexOf( '_' );
        appendKey( ( territoryPosition >= 0 ? withoutModifier.left( territoryPosition ) : withoutModifier ) + modifier );
        appendKey( withoutModifier );
    }

    const int territoryPosition = withoutModifier.indexOf( '_' );
    appendKey( territoryPosition >= 0 ? withoutModifier.left( territoryPosition ) : withoutModifier );

    return keys;
}

Calamares::GeoIP::RegionZonePair
Config::defaultTimezoneForLocale( const QString& localeName ) const
{
    const QStringList keys = localeLookupKeys( localeName );
    for ( const QString& key : keys )
    {
        auto it = m_localeTimezones.constFind( key );
        if ( it != m_localeTimezones.cend() )
        {
            return it.value();
        }
    }
    return m_startingTimezone;
}

bool
Config::hasTimezoneForLocale( const QString& localeName ) const
{
    const QStringList keys = localeLookupKeys( localeName );
    return std::any_of( keys.cbegin(),
                        keys.cend(),
                        [ this ]( const QString& key ) { return m_localeTimezones.contains( key ); } );
}

void
Config::setCurrentLocation()
{
    if ( m_locationSource != LocationSource::User )
    {
        QString language;
        auto* queue = Calamares::JobQueue::instance();
        if ( queue && queue->globalStorage() )
        {
            language = Calamares::Locale::readGS( *queue->globalStorage(), QStringLiteral( "LANG" ) );
        }
        if ( language.isEmpty() )
        {
            language = QLocale().name();
        }
        setCurrentLocation( defaultTimezoneForLocale( language ), LocationSource::Locale );
    }
...
Config::setCurrentLocation( const QString& regionzone )
{
    ...
    if ( r.isValid() )
    {
        setCurrentLocation( r, LocationSource::User );
    }
}
...
{
    setCurrentLocation( tz, LocationSource::User );
}

void
Config::setCurrentLocation( const QString& regionName, const QString& zoneName )
{
    setCurrentLocation( regionName, zoneName, LocationSource::User );
}

void
Config::setCurrentLocation( const Calamares::GeoIP::RegionZonePair& tz, LocationSource source )
{
    setCurrentLocation( tz.region(), tz.zone(), source );
}

void
Config::setCurrentLocation( const QString& regionName, const QString& zoneName, LocationSource source )
{
    using namespace Calamares::Locale;
    auto* zone = m_zonesModel->find( regionName, zoneName );
    if ( zone )
    {
        setCurrentLocation( zone, source );
    }
    else
    {
        auto* fallback = m_zonesModel->find( m_startingTimezone.region(), m_startingTimezone.zone() );
        if ( !fallback )
        {
            fallback = m_zonesModel->find( QStringLiteral( "America" ), QStringLiteral( "New_York" ) );
        }
        setCurrentLocation( fallback, source );
    }
}
...
{
    setCurrentLocation( location, LocationSource::User );
}

void
Config::setCurrentLocation( const Calamares::Locale::TimeZoneData* location, LocationSource source )
{
    if ( !location )
    {
        return;
...
static inline void
getLocaleTimezones( const QVariantMap& configurationMap,
                    QHash< QString, Calamares::GeoIP::RegionZonePair >& localeTimezones )
{
    bool ok = false;
    const QVariantMap map = Calamares::getSubMap( configurationMap, "localeTimezones", ok );
    if ( !ok )
    {
        return;
    }

    localeTimezones.clear();
    for ( auto it = map.cbegin(); it != map.cend(); ++it )
    {
        const auto timezone = Calamares::GeoIP::splitTZString( it.value().toString() );
        if ( timezone.isValid() )
        {
            localeTimezones.insert( it.key().trimmed().toLower(), timezone );
        }
        else
        {
            cWarning() << "Ignoring invalid offline timezone" << it.value().toString() << "for locale" << it.key();
        }
    }
}
...
Config::setConfigurationMap( const QVariantMap& configurationMap )
{
    ...
    getLocaleTimezones( configurationMap, m_localeTimezones );
}
