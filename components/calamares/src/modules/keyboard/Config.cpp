...
static AdditionalLayoutInfo
liveAdditionalLayoutInfo( const QString& layout )
{
    AdditionalLayoutInfo extra = Config::getAdditionalLayoutInfo( layout );
...
Config::cancel()
{
    auto extra = liveAdditionalLayoutInfo( m_original.selectedLayout );
    if ( m_configureXkb )
    {
...
static bool
isRussianLocale( const QString& locale )
{
    return locale.compare( QStringLiteral( "ru" ), Qt::CaseInsensitive ) == 0
        || locale.startsWith( QStringLiteral( "ru_" ), Qt::CaseInsensitive );
}
...
Config::guessLocaleKeyboardLayout()
{
    ...
    if ( !lang.isEmpty() )
    {
        ...
        {
            cDebug() << Logger::SubEntry << "KaskadOS Russian locale" << lang << "uses the US keyboard";
            lang = QStringLiteral( "us" );
        }
        else
        {
