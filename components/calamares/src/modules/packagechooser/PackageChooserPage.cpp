...
void
PackageChooserPage::showPreview( const QPixmap& fallback, const QString& path )
{
    m_previewMovie.reset();
    ui->productScreenshot->setPixmap( fallback );
    ...
    connect( movie.get(), &QMovie::frameChanged, this, [ this ]() {
        if ( m_previewMovie )
        {
            ui->productScreenshot->setPixmap( m_previewMovie->currentPixmap() );
        }
    } );
    m_previewMovie = std::move( movie );
...
PackageChooserPage::currentChanged( const QModelIndex& index )
{
    if ( !index.isValid() || !ui->products->selectionModel()->hasSelection() )
    {
        ...
        showPreview( m_introduction.screenshot, m_introduction.screenshotPath );
    ...
    else
    {
        ...
        else
        {
            showPreview(
                currentScreenshot,
                model->data( index, PackageListModel::ScreenshotPathRole ).toString() );
        }
    }
}
