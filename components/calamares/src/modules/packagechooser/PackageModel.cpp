...
resolveScreenshotPath( const QString& path )
{
    ...
    const QString brandingPath = branding->componentDirectory() + QStringLiteral( "/" ) + path;
    return QFileInfo::exists( brandingPath ) ? brandingPath : QString();
}

static QPixmap
loadScreenshot( const QString& path )
{
    const QString resolvedPath = resolveScreenshotPath( path );
    return resolvedPath.isEmpty() ? QPixmap() : QPixmap( resolvedPath );
}
