...
static QStringList
cleanFullNameParts( const QString& name )
{
...
void
Config::updateAutomaticHostName()
{
    if ( m_customHostName )
    {
        return;
    ...
    const QString hostname
        = makeHostnameSuggestion( m_hostnameTemplate, cleanFullNameParts( fullName() ), loginName() );
...
Config::setFullName( const QString& name )
{
    ...
    if ( name != m_fullName )
    {
        ...
        const QStringList cleanParts = cleanFullNameParts( name );
...
Config::passwordStatus( const QString& pw1, const QString& pw2 ) const
{
    ...
    for ( const auto& pc : m_passwordChecks )
    {
        ...
        if ( !message.isEmpty() )
        {
            return qMakePair( PasswordValidity::Invalid, message );
        }
    }
