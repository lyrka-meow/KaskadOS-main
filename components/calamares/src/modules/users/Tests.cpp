...
void
UserTests::testAutomaticHostname()
{
    QVariantMap hostnameSettings;
    hostnameSettings.insert( "location", "EtcFile" );
    hostnameSettings.insert( "template", "${login}-host" );

    QVariantMap configuration;
    configuration.insert( "hostname", hostnameSettings );

    Config c;
    c.setConfigurationMap( configuration );

    c.setLoginName( "ped" );
    QCOMPARE( c.hostname(), QStringLiteral( "ped-host" ) );

    c.setLoginName( "ivan" );
    QCOMPARE( c.hostname(), QStringLiteral( "ivan-host" ) );

    c.setHostName( "custom-host" );
    c.setLoginName( "alex" );
    QCOMPARE( c.hostname(), QStringLiteral( "custom-host" ) );
}
...
UserTests::testUserPassword()
{
...

    {
        ...
        QVERIFY( c.userPassword().isEmpty() );
        QVERIFY( c.userPasswordSecondary().isEmpty() );
        // Password-strength checks are disabled, but both fields must still match.
        QCOMPARE( c.userPasswordValidity(), int( Config::PasswordValidity::Valid ) );

        c.setUserPassword( "1" );
        QCOMPARE( c.userPasswordValidity(), int( Config::PasswordValidity::Invalid ) );
        c.setUserPasswordSecondary( "1" );
        QCOMPARE( c.userPasswordValidity(), int( Config::PasswordValidity::Valid ) );
