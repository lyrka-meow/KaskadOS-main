...
static const QString s_serviceType = QStringLiteral("KaskadInstaller/Effect");
...
bool ScriptedEffectLoader::loadDeclarativeEffect(const KPluginMetaData &metadata)
{
    ...
    const QString scriptFile = QStandardPaths::locate(QStandardPaths::GenericDataLocation, QLatin1StringView("kaskad-installer/effects/") + name + QLatin1StringView("/contents/ui/main.qml"));
...
{
    return KPackage::PackageLoader::self()->listPackages(s_serviceType, QStringLiteral("kaskad-installer/effects"));
}

KPluginMetaData ScriptedEffectLoader::findEffect(const QString &name) const
{
    const auto plugins = KPackage::PackageLoader::self()->findPackages(s_serviceType, QStringLiteral("kaskad-installer/effects"), [name](const KPluginMetaData &metadata) {
...
PluginEffectLoader::PluginEffectLoader(QObject *parent)
    : AbstractEffectLoader(parent)
    , m_pluginSubDirectory(QStringLiteral("kaskad-installer/effects/plugins"))
{
}
